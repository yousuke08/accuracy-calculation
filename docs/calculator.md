...
局所感度解析は、入力パラメータの微小な変化が出力に与える影響を評価する手法です。ここでは、**相対感度**を用いて感度を表現します。

**相対感度 ($S_x^y$)** は、入力パラメータ ($x$) の相対的な変化に対する出力 ($y$) の相対的な変化の比率として定義されます。

$ S_x^y = \frac{\frac{\Delta y}{y}}{\frac{\Delta x}{x}} = \frac{\partial y}{\partial x} \cdot \frac{x}{y} $
...
- $\Delta y$ は出力の変化量
- $\Delta x$ は入力の変化量
- $\frac{\partial y}{\partial x}$ は $y$ を $x$ で偏微分したもの

このメソッドでは、各入力パラメータを公称値から微小量 (`delta`) だけ変化させ、そのときの出力の変化を数値的に計算することで感度を近似しています。具体的には、中心差分近似を用いて感度を計算しています。

$ S_x^y \approx \frac{\frac{y(x + \Delta x) - y(x - \Delta x)}{y(x)}}{\frac{2 \Delta x}{x}} = \frac{y(x + \Delta x) - y(x - \Delta x)}{y(x)} \cdot \frac{x}{2 \Delta x} $

この計算により、各コンポーネントのばらつきが回路の出力にどの程度影響を与えるかを定量的に評価できます。
